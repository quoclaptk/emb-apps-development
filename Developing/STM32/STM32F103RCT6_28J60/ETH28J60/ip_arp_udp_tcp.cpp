...
uint16_t ip_arp_udp_tcp::checksum(uint8_t *buf, uint16_t len,uint8_t type)
{
	// type 0=ip
	//      1=udp
	//      2=tcp
...
void ip_arp_udp_tcp::init_ip_arp_udp_tcp(uint8_t *mymac,uint8_t *myip,uint8_t wwwp){
        uint8_t i=0;
        wwwport=wwwp;
        while(i<4){
                ipaddr[i]=myip[i];
...
uint8_t ip_arp_udp_tcp::eth_type_is_arp_and_my_ip(uint8_t *buf,uint16_t len){
        uint8_t i=0;
        //
        if (len<41){
                return(0);
...
uint8_t ip_arp_udp_tcp::eth_type_is_ip_and_my_ip(uint8_t *buf,uint16_t len){
        uint8_t i=0;
...
void ip_arp_udp_tcp::make_eth(uint8_t *buf)
{
        uint8_t i=0;
        //
...
void ip_arp_udp_tcp::make_eth_ip_new(uint8_t *buf, uint8_t* dst_mac)
{
        uint8_t i=0;
        //
        //copy the destination mac from the source and fill my mac into src
...
void ip_arp_udp_tcp::fill_ip_hdr_checksum(uint8_t *buf)
{
        uint16_t ck;
...
void ip_arp_udp_tcp::make_ip_tcp_new(uint8_t *buf, uint16_t len,uint8_t *dst_ip)
{
    uint8_t i=0;
...
// make a return ip header from a received ip packet
void ip_arp_udp_tcp::make_ip(uint8_t *buf)
{
        uint8_t i=0;
        while(i<4){
...
void ip_arp_udp_tcp::make_tcphead(uint8_t *buf,uint16_t rel_ack_num,uint8_t mss,uint8_t cp_seq)
{
        uint8_t i=0;
        uint8_t tseq;
        while(i<2){
...
void ip_arp_udp_tcp::make_arp_answer_from_request(uint8_t *buf)
{
        uint8_t i=0;
        //
        make_eth(buf);
        ...
        // eth+arp is 42 bytes:
        eth.enc28j60PacketSend(42,buf);
}

void ip_arp_udp_tcp::make_echo_reply_from_request(uint8_t *buf,uint16_t len)
{
        this->make_eth(buf);
        this->make_ip(buf);
        ...
        //
        eth.enc28j60PacketSend(len,buf);
}

// you can send a max of 220 bytes of data
void ip_arp_udp_tcp::make_udp_reply_from_request(uint8_t *buf,char *data,uint8_t datalen,uint16_t port)
{
        uint8_t i=0;
        uint16_t ck;
        make_eth(buf);
        ...
        eth.enc28j60PacketSend(UDP_HEADER_LEN+IP_HEADER_LEN+ETH_HEADER_LEN+datalen,buf);
}

void ip_arp_udp_tcp::make_tcp_synack_from_syn(uint8_t *buf)
{
        uint16_t ck;
        make_eth(buf);
        // total length field in the IP header must be set:
        ...
        eth.enc28j60PacketSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+4+ETH_HEADER_LEN,buf);
}

// get a pointer to the start of tcp data in buf
// Returns 0 if there is no data
// You must call init_len_info once before calling this function
uint16_t ip_arp_udp_tcp::get_tcp_data_pointer(void)
{
        if (info_data_len){
...
void ip_arp_udp_tcp::init_len_info(uint8_t *buf)
{
...
uint16_t ip_arp_udp_tcp::fill_tcp_data_p(uint8_t *buf,uint16_t pos, const char *progmem_s PROGMEM)
{
        char c;
...
// tcp data. Returns the position at which the string after
// this string could be filled.
uint16_t ip_arp_udp_tcp::fill_tcp_data(uint8_t *buf,uint16_t pos, const char *s)
{
        // fill in tcp data at position pos
        //
        // with no options the data starts after the checksum + 2 more bytes (urgent ptr)
...
void ip_arp_udp_tcp::make_tcp_ack_from_any(uint8_t *buf)
{
        uint16_t j;
        make_eth(buf);
        // fill the header:
        ...
        eth.enc28j60PacketSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+ETH_HEADER_LEN,buf);
}
...
// length and checksum
void ip_arp_udp_tcp::make_tcp_ack_with_data(uint8_t *buf,uint16_t dlen)
{
        uint16_t j;
        // fill the header:
        ...
        buf[TCP_CHECKSUM_H_P]=j>>8;
        buf[TCP_CHECKSUM_L_P]=j& 0xff;
        eth.enc28j60PacketSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+dlen+ETH_HEADER_LEN,buf);
}


/* new functions for web client interface */
void ip_arp_udp_tcp::make_arp_request(uint8_t *buf, uint8_t *server_ip)
{
       uint8_t i=0;
...
uint8_t ip_arp_udp_tcp::arp_packet_is_myreply_arp ( uint8_t *buf )
{
       uint8_t i;
...
// make a  tcp header
void ip_arp_udp_tcp::tcp_client_send_packet(uint8_t *buf,uint16_t dest_port, uint16_t src_port, uint8_t flags, uint8_t max_segment_size,
	uint8_t clear_seqack, uint16_t next_ack_num, uint16_t dlength, uint8_t *dest_mac, uint8_t *dest_ip)
{
      ...
      // check sum
       ...
       buf[TCP_CHECKSUM_L_P]=ck& 0xff;
       // add 4 for option mss:
       eth.enc28j60PacketSend(IP_HEADER_LEN+TCP_HEADER_LEN_PLAIN+dlength+ETH_HEADER_LEN,buf);

}

uint16_t ip_arp_udp_tcp::tcp_get_dlength ( uint8_t *buf )
{
	int dlength, hlength;
