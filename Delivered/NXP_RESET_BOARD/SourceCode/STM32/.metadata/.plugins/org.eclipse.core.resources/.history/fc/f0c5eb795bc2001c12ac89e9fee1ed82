...
void csn(bool level)
{
	if(level)HAL_GPIO_WritePin(CSN_GPIO_Port, CSN_Pin, GPIO_PIN_SET);
	else HAL_GPIO_WritePin(CSN_GPIO_Port, CSN_Pin, GPIO_PIN_RESET);
}
void ce(bool level)
{
	if(level)HAL_GPIO_WritePin(CE_GPIO_Port, CE_Pin, GPIO_PIN_SET);
...
#define onMsg "RELAYON"
#define offMsg "RELAYOFF"
...
int main(void)
{
	...
	while (1)
	{
		radio.write((byte*)onMsg, sizeof(onMsg));
		delay(1000);
		radio.write((byte*)offMsg, sizeof(offMsg));
		delay(1000);
	}
}
