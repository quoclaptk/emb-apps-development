...
{
    beginTransaction();
    status = spiWrite(R_REGISTER | reg);
    while (len--) { *buf++ = spiWrite(0xFF); }
    endTransaction();
}
...
{
    beginTransaction();
    status = spiWrite(W_REGISTER | reg);
    while (len--) { spiWrite(*buf++); }
    endTransaction();
}
...
void RF24::write_payload(const void* buf, uint8_t data_len, const uint8_t writeType)
{
    ...
    beginTransaction();
    status = spiWrite(writeType);
    while (data_len--) { spiWrite(*current++); }
    while (blank_len--) { spiWrite(0); }
    endTransaction();
}
...
void RF24::read_payload(void* buf, uint8_t data_len)
{
    ...
    beginTransaction();
    status = spiWrite(R_RX_PAYLOAD);
    while (data_len--) { *current++ = spiWrite(0xFF); }
    while (blank_len--) { spiWrite(0xff); }
    endTransaction();
}
